...
def rnn(cell, inputs, seq_lens, start_state=None):
    ...
    def condition(i, inputs, state, last_output, outputs):
        return tf.less(i, time)

    # Body of while loop: runs one time step of RNN
    def body(i, inputs, state, last_output, outputs):
        # one RNN time step
        ...
        return [tf.add(i, 1), inputs, new_state, last_output, outputs]
    ...
    # Run RNN while loop
    _, _, last_state, last_output, hidden_states = tf.while_loop(condition, body, \
        loop_vars=[i, inputs, start_state, tf.zeros([batch_size, cell._state_size]), tf.zeros([batch_size, 1, cell._state_size])], \
        ...
        shape_invariants=[i.get_shape(), inputs.get_shape(), start_state.get_shape(), start_state.get_shape(), outputs_shape])
