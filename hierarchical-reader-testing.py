...
alpha_weights = tf.squeeze(tf.batch_matmul(doc_sentences, tf.expand_dims(question_cbow, -1)), [-1])
masked_alpha_weights = tf.mul(alpha_weights, tf.cast(batch_mask, tf.float32))
# top_k.indices; top_k.values
top_k_indices = tf.nn.top_k(masked_alpha_weights, k=1, sorted=True, name="top_k").indices
...
top_k_mask_sentences = tf.reshape(tf.cast(tf.reshape(top_k_mask, [batch_size*max_sent_per_doc, -1]), tf.float32), [-1, 1, 1])

top_k_sentences = tf.mul(masked_sentences, top_k_mask_sentences)
...
print("alpha weights")
print(masked_alpha_weights.eval(feed_dict))
print(alpha_weights.eval(feed_dict).shape)
print("doc sentences")
...
print("h")
print(ha.eval(feed_dict))
print(ha.eval(feed_dict).shape)
print("ha")
print(haha.eval(feed_dict))
print(haha.eval(feed_dict).shape)
